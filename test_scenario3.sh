#!/bin/bash
# Test Scenario 3: Chain Reversal - Backward Circle
# Transfers: 3→2 $3, 2→1 $2, 1→3 $3
# Expected final balances: P1=$9, P2=$21, P3=$30 (Total=$60)

set -e

echo "=========================================="
echo "Testing Scenario 3: Chain Reversal - Backward Circle"
echo "=========================================="

# Build with scenario 3
cp bank_robbery_scenario3.c.bak bank_robbery.c
./build.sh
...
if ! grep -qE "process 3 transferred \\\$ 3 to process 2" events.log; then
    echo "ERROR: Transfer 3→2 \$3 not found"
    exit 1
fi
...
if ! grep -qE "process 1 transferred \\\$ 3 to process 3" events.log; then
    echo "ERROR: Transfer 1→3 \$3 not found"
    exit 1
fi
